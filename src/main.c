...
int rlmain() {
  ...
  while (!WindowShouldClose()) {
    ...
    DrawTextEx(roboto, "GLauncher", (Vector2){glbutton.x + 10, glbutton.y + 15},
               20, 1, WHITE);

    DrawRectangleRec(ascbutton, ascbuttoncol);
    DrawTextEx(roboto, "AS Schedule",
               (Vector2){ascbutton.x + 10, ascbutton.y + 15}, 20, 1, WHITE);

    DrawRectangleRec(scbutton, scbuttoncol);
    DrawTextEx(roboto, "Schedule", (Vector2){scbutton.x + 12, scbutton.y + 15},
               20, 1, WHITE);
